...
/**
 * エージェントに名乗る版。**決めているのは拡張の `package.json` ただ1つ**で、拡張に同梱する
 * ブリッジを束ねるとき（`packages/extension/esbuild.mjs`）に esbuild の `define` で埋め込む。
 * ここに数字を書くと、拡張を上げてもブリッジだけ古い版を名乗る（実際に 0.1.0 の拡張が
 * `0.0.0` を名乗っていた）。束ねていない実行（単体テスト）では `0.0.0-dev`。
 */
declare const __SHOWME_VERSION__: string | undefined;
export const BRIDGE_VERSION: string =
  typeof __SHOWME_VERSION__ === "string" ? __SHOWME_VERSION__ : "0.0.0-dev";
