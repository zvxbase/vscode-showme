...
**VS Code 内蔵のエージェント**（Copilot のエージェントモード）は設定不要。拡張が MCP の提供者
`showme` を自分で登録する（制限モードでは効かない）。

**外部のエージェント**（Claude Code / Codex CLI / Copilot CLI）には一度だけ登録が要る。
コマンドパレットの `ShowMe: エージェント設定を表示` が、3つ分の断片を untitled 文書で開くので、
そこから写す（撤去は `ShowMe: 撤去手順を表示`。どちらも見せるだけで、設定ファイルは書き換えない）。

- 断片は「この拡張と同じ場所に入っている**いちばん新しい ShowMe**を起動する」`node -e` の1行で、
  **拡張を更新しても貼り直さなくてよい**。インストール先は版番号入りのフォルダ
  （`…/extensions/zvxbase.vscode-showme-0.1.0/`）なので、固定のパスを手で貼ると次の版で古いフォルダを指す
- **npm を経路にしない**（`npx` は出さない）。ブリッジは拡張に同梱したものだけを起動する
- 開発中（この repo から動かしている拡張）の文書は、束ね出力のパスを直接起動する断片になる:

```bash
# 開発中に手で登録するときのパス（npm run build で作られる束ね出力）
...
断片の形（文書が実際のパスを埋めて出す。`<BRIDGE>` は開発中のパス）:
...
Copilot CLI と Codex CLI は、この形（Copilot は `"type": "stdio"`）で実際に繋がり、
`list_workspaces` まで届くことを確かめてある（2026-09-15）。
